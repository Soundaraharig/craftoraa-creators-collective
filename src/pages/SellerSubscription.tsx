...
import { ArrowLeft, Check, Crown, User, Image as ImageIcon, Upload } from "lucide-react";
...
const SellerSubscription = () => {
  ...
  const [uploadedFiles, setUploadedFiles] = useState<Record<string, File | null>>({
    "Product Photos": null,
    "Craft Portfolio": null,
    "Workshop Images": null,
  });
        ...
        <section className="px-4 pb-8 space-y-4">
          {registered ? (
            <div className="craft-card p-6 text-center space-y-3">
              ...
              <button
                onClick={() => setActiveTab("plans")}
                className="w-full gradient-warm text-primary-foreground font-body font-semibold text-sm py-3 rounded-lg mt-2"
              >
                View Plans & Upgrade
              </button>
            </div>
            ...
            <>
              ...
              <div className="craft-card p-4 space-y-3">
                ...
                {["Product Photos", "Craft Portfolio", "Workshop Images"].map((label) => (
                  <label
                    key={label}
                    className="flex items-center gap-2 p-3 border border-dashed border-border rounded-lg text-muted-foreground cursor-pointer hover:border-primary transition-colors"
                  >
                    {uploadedFiles[label] ? (
                    ...
                    <span className="text-xs font-body flex-1">
                      {uploadedFiles[label] ? uploadedFiles[label]!.name : label}
                    </span>
                    <input
                      type="file"
                      accept="image/*"
            ...
            return (
              <div key={plan.name} className={`craft-card p-5 relative ${plan.popular ? "ring-2 ring-primary" : ""}`}>
                ...
                <button
                  ...
                  disabled={isCurrent}
                  className={`w-full py-2.5 rounded-lg text-sm font-body font-semibold ${isCurrent ? "bg-muted text-muted-foreground" : "gradient-warm text-primary-foreground"}`}
                >
